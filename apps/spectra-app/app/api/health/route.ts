...
export async function GET(req: Request) {
  ...
  const criticalFailure =
    isProduction() &&
    (supabase.state === 'error' || supabase.state === 'missing' ||
     redis.state    === 'error' || redis.state    === 'missing');

  const httpStatus = criticalFailure ? 503 : 200;

  if (httpStatus !== 200) {
    console.error(JSON.stringify({
      component: 'spectra.api.health', reqId, level: 'error',
      event: 'production_health_degraded', dependencies,
    }));
  } else if (supabase.state !== 'ok' || redis.state !== 'ok') {
    console.warn(JSON.stringify({
      component: 'spectra.api.health', reqId, level: 'warn',
      event: 'non_production_health_degraded', dependencies,
    }));
  }
